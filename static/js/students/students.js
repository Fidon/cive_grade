$(function () {
    // tabs
    $("#container .student_info ul li a").click(function (e) { 
        e.preventDefault();
        var tab_id = $(this).attr('href').replace('#','');
        $("#container .student_info .tab_container .tab_div").each(function () {
    ...
    $("#new_student_form").submit(function (e) { 
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $("#student_cancel_btn").removeClass('d-inline-block').addClass('d-none');
                $("#student_submit_btn").html("<i class='fas fa-spinner fa-pulse'></i> Saving").attr('type', 'button');
            },
            success: function(response) {
                $("#student_cancel_btn").removeClass('d-none').addClass('d-inline-block');
                $("#student_submit_btn").text("Save").attr('type', 'submit');
                ...
                $("#new_student_canvas .offcanvas-body").animate({ scrollTop: 0 }, 'slow');
                $("#new_student_form .formsms").html(fdback);
                
                if(response.update_success) {
                    $("#student_info_div").load(location.href + " #student_info_div");
                } else if(response.success) {
                    $("#new_student_form")[0].reset();
                    students_list_table.draw();
                }
            },
    ...
    $("#students_list_table thead tr").clone(true).attr('class','filters').appendTo('#students_list_table thead');
    var students_list_table = $("#students_list_table").DataTable({
        fixedHeader: true,
        processing: true,
        serverSide: true,
        ajax: {
            url: $("#students_list_url").val(),
            type: "POST",
            data: function (d) {
    ...
    $("#students_search").keyup(function() {
        students_list_table.search($(this).val()).draw();
    });

    $("#students_filter_clear").click(function (e) { 
        e.preventDefault();
        $("#students_search").val('');
        clear_dates();
        students_list_table.search('').draw();
    });

    $("#confirm_delete_btn").click(function (e) { 
        e.preventDefault();
        var formData = new FormData();
        formData.append("delete_student", $("#get_student_id").val());

        $.ajax({
            type: 'POST',
            url: $("#new_student_form").attr('action'),
            data: formData,
            dataType: 'json',
            contentType: false,
            processData: false,

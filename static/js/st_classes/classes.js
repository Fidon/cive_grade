$(function () {
    ...
    $("#container .class_info ul li a").click(function (e) { 
        e.preventDefault();
        var tab_id = $(this).attr('href').replace('#','');
        $("#container .class_info .tab_container .tab_div").each(function () {
    ...
    $("#new_class_form").submit(function (e) { 
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $("#class_cancel_btn").removeClass('d-inline-block').addClass('d-none');
                $("#class_submit_btn").html("<i class='fas fa-spinner fa-pulse'></i> Saving").attr('type', 'button');
            },
            success: function(response) {
                $("#class_cancel_btn").removeClass('d-none').addClass('d-inline-block');
                $("#class_submit_btn").text("Save").attr('type', 'submit');
                ...
                $("#new_class_canvas .offcanvas-body").animate({ scrollTop: 0 }, 'slow');
                $("#new_class_form .formsms").html(fdback);
                
                if(response.update_success) {
                    $("#class_info_div").load(location.href + " #class_info_div");
                } else if(response.success) {
                    $("#new_class_form")[0].reset();
                    classes_list_table.draw();
                }
            },
    ...
    $("#classes_list_table thead tr").clone(true).attr('class','filters').appendTo('#classes_list_table thead');
    var classes_list_table = $("#classes_list_table").DataTable({
        fixedHeader: true,
        processing: true,
        serverSide: true,
        ajax: {
            url: $("#classes_list_url").val(),
            type: "POST",
            data: function (d) {
    ...
    $("#classes_search").keyup(function() {
        classes_list_table.search($(this).val()).draw();
    });

    $("#classes_filter_clear").click(function (e) { 
        e.preventDefault();
        $("#classes_search").val('');
        clear_dates();
        classes_list_table.search('').draw();
    });
    ...
    $("#confirm_delete_btn").click(function (e) { 
        ...
        formData.append("delete_class", $("#get_class_id").val());

        $.ajax({
            type: 'POST',
            url: $("#new_class_form").attr('action'),
            data: formData,
            dataType: 'json',
            contentType: false,
            processData: false,
            ...
            success: function(response) {
                if(response.success) {
                    window.alert('Program/class deleted..!');

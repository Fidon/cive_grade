...
class TableExtractor:
    ...
    def store_process_image(self, file_name, image):
        path = os.path.join(settings.MEDIA_ROOT, r"process_images\table_extractor", file_name)
...
class TableLinesRemover:
    ...
    def store_process_image(self, file_name, image):
        path = os.path.join(settings.MEDIA_ROOT, r"process_images\table_lines_remover", file_name)
        cv2.imwrite(path, image)
...
class OcrToTableTool:
    ...
    #             image_slice_path = os.path.join(settings.MEDIA_ROOT, r"process_images\ocr_slices\img_" + str(image_number) + ".jpg")
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            ...
            for bounding_box in row:
                ...
                cv2.imwrite(image_slice_path, cropped_image)
                results_from_ocr = self.get_result_from_tersseract(image_slice_path)
                row_data.append(results_from_ocr)
                image_number += 1
            self.table.append(row_data)


    def get_result_from_tersseract(self, image_path):
        output = subprocess.getoutput('tesseract ' + image_path + ' - -l eng --oem 3 --psm 7 --dpi 72 -c tessedit_char_whitelist="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-/# "')
        output = output.strip()
        return output
    ...
    def store_process_image(self, file_name, image):
        path = os.path.join(settings.MEDIA_ROOT, r"process_images\ocr_table_tool", file_name)
        cv2.imwrite(path, image)